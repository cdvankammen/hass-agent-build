...
namespace HASS.Agent.Media
{
    internal static class MediaManager
    {
        ...
        internal static async Task InitializeAsync()
        {
            ...
            // if we're not on Windows, use the Linux player adapter path
            if (!OperatingSystem.IsWindows())
            {
                Log.Information("[MEDIA] Running on non-Windows, using Linux PlayerAdapter");

                if (!PlayerAdapter.IsPlayerAvailable())
                {
                    Log.Warning("[MEDIA] 'mpv' not found on system, Linux media playback will be unavailable");
                }
                ...
                Log.Information("[MEDIA] Ready (linux)");
                return;
            }
            ...
            try
            {
                switch (command)
                {
                    ...
                    case MediaPlayerCommand.Play:
                        ...
                        if (!OperatingSystem.IsWindows()) PlayerAdapter.Resume(); else MediaManagerCommands.Play();
                        break;
                    ...
                    case MediaPlayerCommand.Pause:
                        ...
                        if (!OperatingSystem.IsWindows()) PlayerAdapter.Pause(); else MediaManagerCommands.Pause();
                        break;

                    case MediaPlayerCommand.Stop:
                        if (Variables.ExtendedLogging) Log.Information("[MEDIA] Command received: Stop");
                        if (!OperatingSystem.IsWindows()) PlayerAdapter.Stop(); else MediaManagerCommands.Stop();
                        break;
        ...
        internal static async void ProcessMedia(string mediaUri)
        {
            ...
            try
            {
                ...
                // handle tts: URIs (tts:Hello world)
                if (localFile.StartsWith("tts:", StringComparison.OrdinalIgnoreCase) && !OperatingSystem.IsWindows())
                {
                    var text = localFile.Substring(4);
                    var ttsOk = await HASS.Agent.Platform.Linux.Audio.TtsAdapter.SpeakAsync(text);
                    if (!ttsOk) Log.Error("[MEDIA] TTS failed on Linux (espeak/spd-say not available)");
                    return;
                }

                // non-windows playback via PlayerAdapter
                if (!OperatingSystem.IsWindows())
                {
                    var played = await PlayerAdapter.PlayAsync(localFile);
                    if (!played) Log.Error("[MEDIA] Unable to play media on Linux (mpv missing or failed)");
                    else if (Variables.ExtendedLogging) Log.Information("[MEDIA] Playing via mpv: {file}", Path.GetFileName(localFile));
                    return;
                }
