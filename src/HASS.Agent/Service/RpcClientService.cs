...
namespace HASS.Agent.Service
{
    ...
    {
        // On Windows we use named pipes/gRPC, on Linux use PlatformFactory's RPC client (TCP/UDS)
        ...
        internal async Task<(bool success, string error)> ClearEntitiesAsync()
        {
            try
            {
                if (_rpcImpl is HASS.Agent.Platform.IRpcClient platformRpc)
                {
                    var ok = await platformRpc.ClearEntitiesAsync(Variables.AppSettings.ServiceAuthId);
