    ...
    internal class SettingsManager
    {
        ...
        private static bool LoadAppSettings()
        {
            try
            {
                ...
                // load the hotkey (only on Windows)
        ...
        internal static bool GetExtendedLoggingSetting()
        {
            try
            {
                if (!OperatingSystem.IsWindows())
                {
                    var store = HASS.Agent.Platform.PlatformFactory.GetSettingsStore();
                    return store.Get("ExtendedLogging", "0") == "1";
                }
        ...
        internal static void SetExtendedLoggingSetting(bool enabled)
        {
            try
            {
                ...
                {
                    var store = HASS.Agent.Platform.PlatformFactory.GetSettingsStore();
                    store.Set("ExtendedLogging", enabled ? "1" : "0");
                    return;
                }
        ...
        internal static bool GetDpiWarningShown()
        {
            try
            {
                ...
                {
                    var store = HASS.Agent.Platform.PlatformFactory.GetSettingsStore();
                    return store.Get("DpiWarningShown", "0") == "1";
                }
        ...
        internal static void SetDpiWarningShown(bool shown)
        {
            try
            {
                ...
                {
                    var store = HASS.Agent.Platform.PlatformFactory.GetSettingsStore();
                    store.Set("DpiWarningShown", shown ? "1" : "0");
                    return;
                }
        ...
        internal static string DeviceSerialNumber()
        {
            ...
            {
                var store = HASS.Agent.Platform.PlatformFactory.GetSettingsStore();
                var guid = store.Get("DeviceSerialNumber", string.Empty);
                if (string.IsNullOrEmpty(guid))
                {
                    guid = Guid.NewGuid().ToString();
                    store.Set("DeviceSerialNumber", guid);
                }

                return guid;
            }

            var guidWin = (string)Registry.GetValue(Variables.RootRegKey, "DeviceSerialNumber", string.Empty);
            if (string.IsNullOrEmpty(guidWin))
            {
                guidWin = Guid.NewGuid().ToString();
                StoreDeviceSerialNumber(guidWin);
            }
        ...
        internal static bool GetHideDonateButton()
        {
            try
            {
                ...
                {
                    var store = HASS.Agent.Platform.PlatformFactory.GetSettingsStore();
                    return store.Get("HideDonateButton", "0") == "1";
                }
        ...
        internal static void SetHideDonateButton(bool hide)
        {
            try
            {
                ...
                {
                    var store = HASS.Agent.Platform.PlatformFactory.GetSettingsStore();
                    store.Set("HideDonateButton", hide ? "1" : "0");
                    return;
                }
