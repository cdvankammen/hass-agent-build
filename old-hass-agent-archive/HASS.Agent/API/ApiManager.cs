...
namespace HASS.Agent.API
{
    ...
    internal static class ApiManager
    {
            ...
            try
            {
                ...
                using (Variables.LocalApiServer = RestServerBuilder.From<ApiConfiguration>().Build())
                {
                    Variables.LocalApiServer.AfterStarting += (s) =>
                    {
                        ...
                        // commands list route
                        var listCommandsRoute = new Route(ApiEndpoints.ListCommandsRoute, "Get", "/commands", true, "ListCommandsRoute");
                        s.Router.Register(listCommandsRoute);

                        // execute command route
                        var executeCommandRoute = new Route(ApiEndpoints.ExecuteCommandRoute, "Post", "/command", true, "ExecuteCommandRoute");
                        s.Router.Register(executeCommandRoute);

                            // service control routes
                            var svcStatus = new Route(ApiEndpoints.ServiceStatusRoute, "Get", "/service/status", true, "ServiceStatusRoute");
                            s.Router.Register(svcStatus);
                            var svcStart = new Route(ApiEndpoints.ServiceStartRoute, "Post", "/service/start", true, "ServiceStartRoute");
                            s.Router.Register(svcStart);
                            var svcStop = new Route(ApiEndpoints.ServiceStopRoute, "Post", "/service/stop", true, "ServiceStopRoute");
                            s.Router.Register(svcStop);
        ...
        internal static bool ExecuteElevatedPortReservation()
        {
            ...
            {
                Log.Information("[LOCALAPI] Port reservation not required on non-Windows platforms");
                return true;
            }
