...
namespace HASS.Agent.HomeAssistant
{
    ...
    internal static class HassApiManager
    {
        ...
        private static bool CheckSettings()
        {
            try
            {
                if (string.IsNullOrWhiteSpace(Variables.AppSettings.HassUri)) return false;
                if (string.IsNullOrWhiteSpace(Variables.AppSettings.HassToken)) return false;

                // validate URI
                if (!Uri.TryCreate(Variables.AppSettings.HassUri, UriKind.Absolute, out var uri)) return false;
                if (uri.Scheme != Uri.UriSchemeHttp && uri.Scheme != Uri.UriSchemeHttps) return false;

                // basic token heuristic: ensure length is reasonable (Home Assistant tokens are long)
                if (Variables.AppSettings.HassToken.Length < 8) return false;
