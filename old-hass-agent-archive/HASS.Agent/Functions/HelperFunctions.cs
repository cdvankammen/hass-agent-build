    ...
    internal static class HelperFunctions
    {
        ...
        internal static bool ParseMultipleKeys(string keyString, out List<string> keys, out string errorMsg)
        {
            ...
            try
            {
                ...
                // Use regex to match content inside non-escaped brackets. This pattern captures sequences like [content]
                var pattern = new Regex(@"(?<!\\)\[(.*?)(?<!\\)\]");
                var matches = pattern.Matches(keyString);
                foreach (Match m in matches)
                {
                    var group = m.Groups[1].Value;
                    // restore escaped brackets inside
