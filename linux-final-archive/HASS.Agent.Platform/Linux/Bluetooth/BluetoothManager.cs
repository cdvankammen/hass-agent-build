...
namespace HASS.Agent.Platform.Linux.Bluetooth
{
    public class BluetoothManager
    {
        ...
        {
            Log.Information("[Bluetooth] Start scanning (bluetoothctl/DBus)");
            try
            {
                ...
                var psi = new ProcessStartInfo { FileName = "bluetoothctl", Arguments = "scan on", UseShellExecute = false };
                Process.Start(psi)?.Dispose();
            }
            catch (Exception ex)
            {
                Log.Warning(ex, "[Bluetooth] bluetoothctl/DBus scan on failed");
            }
        }

        public void StopScan()
        {
            Log.Information("[Bluetooth] Stop scanning (bluetoothctl/DBus)");
            try
            {
                if (_bluez != null && _bluez.DbUsAvailable)
                {
                ...
                var psi = new ProcessStartInfo { FileName = "bluetoothctl", Arguments = "scan off", UseShellExecute = false };
                Process.Start(psi)?.Dispose();
            }
            catch (Exception ex)
            {
                Log.Warning(ex, "[Bluetooth] bluetoothctl/DBus scan off failed");
            }
        }
