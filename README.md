...
The desktop app also includes a built-in Help tab with quick-start steps and source build/run examples if you prefer staying inside the UI.
...
This repository now contains a headless Linux target and packaging helpers. See `README-linux.md` for build and install instructions and `CHANGELOG.md` for a summary of recent Linux-focused changes.
